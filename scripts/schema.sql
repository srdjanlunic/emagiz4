...
CREATE TABLE SystemVulnerabilities (
                                    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
                                    vulnerability_id UUID REFERENCES Vulnerability(id) ON DELETE CASCADE,
                                    system_id UUID REFERENCES ITSystem(id) ON DELETE CASCADE,
...
CREATE TABLE Settings (
    key TEXT NOT NULL,
    value TEXT,
    user_id UUID NOT NULL REFERENCES UserAccount(id) ON DELETE CASCADE,
    PRIMARY KEY (key, user_id)
);

CREATE TABLE Escalations (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    system_vulnerability_id UUID NOT NULL REFERENCES SystemVulnerabilities(id) ON DELETE CASCADE,
    security_officer_id UUID REFERENCES UserAccount(id),
